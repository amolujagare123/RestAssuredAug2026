package GoRest;

import io.restassured.RestAssured;
import org.hamcrest.Matchers;

import static io.restassured.RestAssured.given;
import static payload.GoRestPayload.GoRestCreateUpdateUser.getCreateUserPayload;

public class UpdateUserWithPayload {

    public static void main(String[] args) {

        RestAssured.baseURI = "https://gorest.co.in/";

        given().log().all()
              //  .header("Authorization","Bearer b6937dfecc6168181b06cde7d79fc22a46f97cc743642ccfe11f93ec68a310f3")
                .auth().oauth2("b6937dfecc6168181b06cde7d79fc22a46f97cc743642ccfe11f93ec68a310f3")
                .header("Content-Type","application/json")
                .body(getCreateUserPayload("vaishali gadekar","vaishali2@gmail.com","female","active"))
                .when().put("/public/v2/users/8593320")
                .then().log().all().statusCode(200)
              //  .time(Matchers.lessThan(4500l))
                .body("email", Matchers.equalTo("vaishali@gmail.com"));
    }
}
